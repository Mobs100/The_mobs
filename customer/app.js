...
const money = value =>
  `${Number(value || 0).toFixed(2).replace(/\.00$/, '')} SAR`;
...
function updateCount(){
  ...
  document
    .querySelectorAll('#navCount,#count')
...
function go(id){
  ...
  if(id === 'tracking' && currentOrder?.tracking_token){
    loadTracking(currentOrder.tracking_token);
  }
...
async function loadData(){
  ...
  try{
    ...
    const message =
      'Could not load the menu. Check Supabase table permissions.';
...
function renderCategories(){
  ...
  el.innerHTML =
    `
      <button
        class="cat ${selectedCategory === 'all' ? 'active' : ''}"
        data-category="all"
    ...
    categories
      .map(c => `
        <button
          class="cat ${selectedCategory === c.id ? 'active' : ''}"
...
function filterCategory(id, button){
  ...
  document
    .querySelectorAll('#categories .cat')
...
function openProduct(productOrName){
  ...
  const art =
    document.querySelector('#product .product-art');

  if(art){

    art.innerHTML = product.image_url
...
function addToCart(){
  ...
  const existing =
    cart.find(
      x => String(x.product_id) === String(currentProduct.id)
    );
...
function validateCustomerInfo(
  ...
  if(!/^05\d{8}$/.test(phone)){
    ...
    if(showMessage){
      ...
      alert(
        'Please enter a valid Saudi mobile number with exactly 10 digits, starting with 05.'
      );
    }
  ...
  if(
    ...
    if(showMessage){
      ...
      alert(
        'Please choose your delivery location on the map or use My Location.'
      );
    }
...
async function placeOrder(){
  ...
  try{
    ...
    // Refresh products before creating order.
    await loadData();
    ...
    // IMPORTANT:
    // Supabase order_items requires product_price NOT NULL.
    // -----------------------------------------------------
    ...
    const normalized =
      cart.map(x => {
        ...
        if(
          ...
          throw new Error(
            `Product "${x.name}" is no longer available.`
          );
        }
        ...
        return {
          ...
          // IMPORTANT:
          // This fixes:
          // null value in column "product_price"
          product_price:
            price,

          // Keep unit_price too in case
          // the table already contains it.
          unit_price:
            price,
    ...
    const deliveryFee =
      cart.length
        ? Number(
            window.MOBS_CONFIG.deliveryFee
          )
        : 0;
    ...
    const address =
      delivery.address ||
      `Map location: ${delivery.latitude},${delivery.longitude}`;
    ...
    //
    // IMPORTANT FIX:
    // product_price is explicitly sent.
    // -----------------------------------------------------
...
async function fetchOrderByTrackingToken(
  token
){
...
async function loadOrders(
  ...
  for(
    const savedOrder of saved
  ){

    try{

      const live =
        await fetchOrderByTrackingToken(
          savedOrder.tracking_token
        );
...
async function openTrackingByToken(
  token
){
...
async function startCustomerOrderRealtime(
  order
){
      ...
      .on(
        ...
        async () => {
          ...
          try{
            ...
            console.warn(
              'Customer realtime refresh failed:',
              error
            );
          }
        }
...
async function loadTracking(
  ...
  try{
    ...
    await startCustomerOrderRealtime(
      currentOrder
    );
